...
NOTE: Sie können integrierte Konten nicht entfernen oder umbenennen. Wenn ein Administrator das Konto entfernt, wird das integrierte Konto beim Neustart wiederhergestellt. *NetApp empfiehlt*, nicht benötigte integrierte Konten mit dem Befehl `lock` zu sperren.

Obwohl verwaiste Konten ein erhebliches Sicherheitsrisiko darstellen, *NetApp empfiehlt dringend*, die Auswirkungen des Entfernens von Konten aus dem lokalen Kontenspeicher zu testen.
